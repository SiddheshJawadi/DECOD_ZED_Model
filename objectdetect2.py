...
def load_depth_into_numpy_array(depth):
    ...
    return np.array(ar).reshape((im_height, im_width, channels)).astype(float)
...
depth_np_global = np.zeros([width, height, 4], dtype=float)
...
def main(args):
   ...
   # MODEL_NAME = 'ssd_mobilenet_v1_coco_2018_01_28'
    ...
    #    print(DOWNLOAD_BASE + MODEL_FILE)
     #   MODEL_LINK = 'http://download.tensorflow.org/models/object_detection/ssd_mobilenet_v1_coco_2018_01_28.tar.gz'

 #       req = urllib.request.Request(MODEL_LINK)
  #   for file in tar_file.getmembers():
#            file_name = os.path.basename(file.name)
 #             tar_file.extract(file, 'data/')
    dload.save('http://download.tensorflow.org/models/object_detection/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03.tar.gz', 'D:/ZED_SDK/samples/body tracking/python/model2.tar.gz')
    file = tarfile.open('D:/ZED_SDK/samples/body tracking/python/model2.tar.gz')
    file.extractall('D:/ZED_SDK/samples/body tracking/python')
    file.close()
    # List of the strings that is used to add correct label for each box.
    PATH_TO_LABELS = os.path.join('D:/ZED_SDK/samples/body tracking/python/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03/data', 'mscoco_label_map.pbtxt')
    NUM_CLASSES = 90
    ...
    print("Loading model " + 'ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03')
    detection_graph = tf.Graph()
    with detection_graph.as_default():
        od_graph_def = tf.compat.v1.GraphDef()
        with tf.compat.v2.io.gfile.GFile('D:/ZED_SDK/samples/body tracking/python/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03' + '/frozen_inference_graph.pb', 'rb') as fid:
