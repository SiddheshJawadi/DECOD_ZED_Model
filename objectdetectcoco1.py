...
def capture_thread_func(svo_filepath=None):
    ...
    while not exit_signal:
        if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
            ...
            #zed.retrieve_image(image_mat, sl.VIEW.RIGHT, resolution=image_size)
...
#            file_name = os.path.basename(file.name)
 #             tar_file.extract(file, 'data/')
    dload.save('http://download.tensorflow.org/models/object_detection/ssd_mobilenet_v1_coco_2018_01_28.tar.gz', 'D:/ZED_SDK/samples/body tracking/python/model.tar.gz')
    file = tarfile.open('D:/ZED_SDK/samples/body tracking/python/model.tar.gz')
    ...
    print("Loading model " + 'ssd_mobilenet_v1_coco_2018_01_28')
    detection_graph = tf.Graph()
    with detection_graph.as_default():
        od_graph_def = tf.compat.v1.GraphDef()
        with tf.compat.v2.io.gfile.GFile('D:/ZED_SDK/samples/body tracking/python/ssd_mobilenet_v1_coco_2018_01_28' + '/frozen_inference_graph.pb', 'rb') as fid:
